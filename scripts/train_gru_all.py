"""Train GRU on all 4 C-MAPSS datasets."""
import sys
...
def train_all_datasets():
    ...
    print("\n" + "="*90)
    print(" "*25 + "GRU TRAINING ON ALL C-MAPSS DATASETS")
    print("="*90)
    ...
    for i, dataset_id in enumerate(DATASETS, 1):
        ...
        print(f"  Conditions: {DATASET_INFO[dataset_id]['conditions']} | Faults: {DATASET_INFO[dataset_id]['faults']} | Engines: {DATASET_INFO[dataset_id]['engines']}")
        print(f"{'='*90}\n")
        
        dataset_start = time.time()
        
        try:
            test_metrics = train_gru(dataset_id)
            results[dataset_id] = {
            ...
            print(f"\n✅ {dataset_id} complete in {dataset_time/60:.1f} min")
            
        except Exception as e:
            print(f"\n❌ {dataset_id} failed: {e}")
            results[dataset_id] = {'rmse': None, 'mae': None, 'nasa_score': None, 'status': f'FAILED'}
    
    # Summary
    total_time = time.time() - start_time
    print("\n" + "="*90)
    print(" "*30 + "GRU RESULTS SUMMARY")
    print("="*90)
    print(f"Dataset | Difficulty | Cond | Faults | Engines | RMSE      | MAE       | NASA Score")
    print("-"*90)
    ...
    for dataset_id in DATASETS:
        ...
        if res['status'] == 'SUCCESS':
            print(f"{dataset_id:7} | {info['difficulty']:10} | {info['conditions']:4} | "

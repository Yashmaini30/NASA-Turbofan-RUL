...
class GRUTrainer:
    ...
    def __init__(self, dataset='FD001', sequence_length=30, config=None):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train GRU model for NASA C-MAPSS RUL prediction')
    parser.add_argument('--dataset', default='FD001', choices=['FD001', 'FD002', 'FD003', 'FD004'],
                       help='Dataset to use (default: FD001)')
    parser.add_argument('--sequence-length', type=int, default=30,
                       help='Sequence length for time series (default: 30)')
    ...
    if results:
        print(f"\\n✅ Success! RMSE: {results['rmse']:.2f} cycles")
    else:
        print("\\n❌ Training failed!")

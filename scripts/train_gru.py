"""GRU training pipeline - minimal, production-grade implementation."""
import sys
...
class Trainer:
    """Minimal GRU trainer with early stopping and checkpointing."""
...
def main(dataset_id='FD001'):
    """
    Train GRU model on specified C-MAPSS dataset.
    ...
    model = create_gru_model(input_dim, config['model']['gru'])
    
    # Train
    print("\n=== Training ===")
    Path('models/gru').mkdir(parents=True, exist_ok=True)
    save_path = f'models/gru/best_model_{dataset_id}.pth'
    ...
    # Save results
    np.savez(f'models/gru/results_{dataset_id}.npz',
